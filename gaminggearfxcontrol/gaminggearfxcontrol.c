...
int main(int argc, char **argv) {
	...
	if (parameter_single) {
		...
		if (parameter_color) {
			...
			if (gfx_result != GFX_SUCCESS) {
				g_warning(_("There was an error setting colors"));
				goto exit_2;
			}
		}
		...
		if (gfx_result != GFX_SUCCESS) {
			g_warning(_("There was an error resetting colors"));
			goto exit_2;
		}
	}
	...
	if (parameter_all || parameter_single || parameter_reset) {
		...
		if (gfx_result != GFX_SUCCESS) {
			g_warning(_("There was an error updating"));
			goto exit_2;
		}
	}
