...
static GaminggearMacroKeystrokes *steelseries_events_to_gaminggear_macro_keystrokes(gchar const *string) {
	...
	while (*event) {
		...
		case STEELSERIES_MACRO_EVENT_TYPE_WAIT:
			if (index == 0)
				g_warning(_("steelseries_events_to_gaminggear_macro_keystrokes: wait without first event"));
			else
				gaminggear_macro_keystroke_set_period(&keystrokes->keystrokes[index - 1], get_uint(extra_regex, *event));
			break;
		default:
			g_warning(_("steelseries_events_to_gaminggear_macro_keystrokes: unknown type %u"), type);
			break;
		}
...
GaminggearMacros *macros_conversions_steelseries_engine3_import(gchar const *filename, GError **error) {
	...
	if (retval != SQLITE_OK) {
		g_set_error(error, G_FILE_ERROR_FAILED, G_FILE_ERROR_FAILED, _("sqlite3 could not exec statement %s"), message);
