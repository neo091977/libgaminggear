...
int gaminggear_device_open(GaminggearDevice *gaminggear_device, gchar const *key, gint flags, GError **error) {
	...
	if (!path) {
		g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_INVAL, _("GaminggearDevice: error opening file for key %s: no path"), key);
		return -1;
	}

	fd = open(path, flags, 0);
	if (fd < 0)
		g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(errno), _("GaminggearDevice: error opening %s for key %s: %s"), path, key, g_strerror(errno));
...
gboolean gaminggear_device_close(GaminggearDevice *gaminggear_device, gchar const *key, GError **error) {
	...
	if (!g_hash_table_lookup_extended(gaminggear_device->priv->fds, key, NULL, &value)) {
		g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_INVAL, _("GaminggearDevice: error closing fd for key %s: no entry"), key);
		return FALSE;
	}

	fd = GPOINTER_TO_INT(value);
	if (close(fd) < 0) {
		g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(errno), _("GaminggearDevice: error closing fd for key %s: %s"), key, g_strerror(errno));
		return FALSE;
	}
...
static void destroy_fd(gpointer data) {
	...
	if (close(fd) < 0)
		g_warning(_("GaminggearDevice: error closing fd %i: %s"), fd, g_strerror(errno));
}
