...
gpointer g_gaminggear_key_file_get_binary(GKeyFile *key_file, gchar const *group_name, gchar const *key, gsize size, GError **error) {
	...
	if (strlen(string) != size * 2) {
		g_set_error(error, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_INVALID_VALUE, _("Data has wrong length %lu instead of %lu"), (gulong)strlen(string), (gulong)(size * 2));
		g_free(string);
		return NULL;
	}
...
static gboolean gaminggear_create_dir_if_needed_with_mode(gchar const *dir, int mode, GError **error) {
	if (!g_file_test(dir, G_FILE_TEST_IS_DIR)) {
		if (g_mkdir_with_parents(dir, mode)) {
			g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(errno), _("Error creating directory: %s"), g_strerror(errno));
			return FALSE;
		}
	}
	return TRUE;
